...
console.info('CASE copyWithin from Curray')
{
    const letters = new Curray('a', 'b', 'c', 'd', 'e')
    const copyLetters = letters.copyWithin(0, 3, 4)
...
{
    const letters = new Curray('a', 'b', 'c', 'd', 'e')
    const copyLetters = (letters.copyWithin(1, 3));
    console.assert(letters.length === 5, 'letters length is 5')
    console.assert(letters[0] === 'a', 'letters at 0 is a')
