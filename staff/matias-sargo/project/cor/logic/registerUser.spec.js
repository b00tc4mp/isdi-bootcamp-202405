...
describe('registerUser', () => {
    ...
    it('succeeds on new user', () =>
        registerUser('John', 'Doe', 'john.doe@egmail.com', 'johndoe', 'password123', 'password123')
    ...
    it('fails on existing user with same email', () => {
        ...
        return User.create({ name: 'John', surname: 'Doe', email: 'john.doe@egmail.com', username: 'johndoe', password: 'password123' })
            .then(() => registerUser('Jane', 'Smith', 'john.doe@egmail.com', 'janesmith', 'password123', 'password123'))
    ...
    it('fails on existing user with same username', () => {
        ...
        return User.create({ name: 'John', surname: 'Doe', email: 'john.doe@egmail.com', username: 'johndoe', password: 'password123' })
            .then(() => registerUser('Jane', 'Smith', 'jane.smith@egmail.com', 'johndoe', 'password123', 'password123'))
            .catch(error => _error = error)
            .finally(() => {
                expect(_error).to.be.instanceOf(DuplicityError)
                expect(_error.message).to.equal('user already exists')
