...
const editUserInfo = (oldUsername, avatar, newUsername, password) => {
    ...
    const newUser = data.findUser(user => user.username === newUsername)

    if (newUser) throw new Error('username already exists')
    ...
    if (newUsername && user.username !== newUsername) {
        const posts = data.findPosts(post => post.author.username === user.username)
