...
function validateUsername(username) {
    ...
    if (!USERNAME_REGEX.test(username)) throw new SyntaxError('invalid username')
        if (username.trim().length < 4) throw new SyntaxError('invalid username')
...
function validateName(name, explain = 'name') {
    ...
    if (!NAME_REGEX.test(name)) throw new SyntaxError(`invalid ${explain}`)
        if (name.trim() === '') throw new SyntaxError('invalid name')
}
...
function validateImage(image){
    ...
    if (!image.startsWith('http')) throw new SyntaxError('invalid image')
