{
  const removeLike = (postId) => {
    ...
    if (!Array.isArray(post.likeUsers)) {
      post.likeUsers = [];
    }

    const usernameIndex = post.likeUsers.findIndex(
      (username) => username === user.username
    );

    if (usernameIndex !== -1) post.likeUsers.splice(usernameIndex, 1);

    localStorage.posts = JSON.stringify(posts);
  };

  logic.removeLike = removeLike;
}
