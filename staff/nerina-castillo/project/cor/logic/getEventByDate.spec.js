...
describe('getEventByDate', () => {
    ...
    it('succeeds on existing user returning events for a date', () => {
        ...
        return User.create({ name: 'gon', username: 'gonzalo', role: 'user', email: 'gon@zalo.com', password: 'gonzalo123' })
            .then(user =>
                Event.create({ author: user.id, image: 'https://media.giphy.com/media/gHbQG42yJMVHy/giphy.gif', title: 'Barrenfields', description: 'Barrenfields concert', location: { type: 'Point', coordinates: [40.7128, -74.0060] }, startDate: new Date(date), startTime: '21:30', tickets: 'http://crocantickets.com' })
                    .then(() => {
                        Event.create({ author: user.id, image: 'https://media.giphy.com/media/gHbQG42yJMVHy/giphy.gif', title: 'Luis Aragofest', description: 'Punk en las canchas', location: { type: 'Point', coordinates: [40.7128, -74.0060] }, startDate: new Date(date), startTime: '22:30', tickets: 'http://crocantickets.com' })
                            .then(() => {
    ...
    it('succeeds on existing user following the event author', () => {
        ...
        return User.create({ name: 'followed', username: 'followedUser', role: 'user', email: 'followed@user.com', password: 'followed123' })
            .then(_followedUser =>
                User.create({ name: 'gon', username: 'gonzalo', role: 'user', email: 'gon@zalo.com', password: 'gonzalo123', following: [_followedUser.id] }))
            .then(user =>
                Event.create({ author: user.following[0], image: 'https://media.giphy.com/media/gHbQG42yJMVHy/giphy.gif', title: 'Barrenfields', description: 'Barrenfields concert', location: { type: 'Point', coordinates: [40.7128, -74.0060] }, startDate: new Date(date), startTime: '21:30', tickets: 'http://crocantickets.com' })
                    .then(() => getEventByDate(user.id, date))
                    .then(events => {
                        expect(events).to.have.lengthOf(1)
                        expect(events[0].author.id).to.equal(user.following[0].toString())
                        expect(events[0].author.username).to.equal('followedUser')
                        expect(events[0].author.avatar).to.equal('./music.png')
    ...
    it('fails on event with non-existing author', () => {
        ...
        return User.create({ name: 'gon', username: 'gonzalo', role: 'user', email: 'gon@zalo.com', password: 'gonzalo123' })
            .then(user => {
                return Event.create({ author: new ObjectId().toString(), image: 'https://media.giphy.com/media/gHbQG42yJMVHy/giphy.gif', title: 'Barrenfields', description: 'Barrenfields concert', location: { type: 'Point', coordinates: [40.7128, -74.0060] }, startDate: new Date(date), startTime: '21:30', tickets: 'http://crocantickets.com' })
                    .then(() => getEventByDate(user.id, date))
                    .catch(error => {
                        _error = error
                    })
