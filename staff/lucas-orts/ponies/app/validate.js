...
function validateObject(object, explain = 'object') {
    if (object === null || typeof object !== 'object' || object.consturctor !== Object) throw new TypeError(`${explain} is not an object`)
}
...
function validatePassword(password) {
    ...
    if (password.trim().length < 8) throw new SyntaxError('password length is lower than 8 characters ')
    if (password.includes(' ')) throw new SyntaxError('paasword has empty sapece')
}
...
function validateEmail(email) {
    ...
    if (!EMAIL_REGEX.test(email)) throw new SyntaxError(`invalud ${email}`)
}
...
function validateDate(date) {
    ...
    if (date.trim().length === 0) throw new Error('invalid date')
}
