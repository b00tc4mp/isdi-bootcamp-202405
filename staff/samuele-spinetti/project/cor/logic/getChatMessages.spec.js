...
describe('getChatMessages', () => {
    ...
    it('succeeds on existing user and listing messages', () => {
        debugger
        return User.create({ name: 'Samu', surname: 'Spine', username: 'samu', email: 'samu@spine.com', password: '123123123' })
            .then(user =>
                User.create({ name: 'Liliana', surname: 'Deponte', username: 'lili', email: 'lili@deponte.com', password: '123123123' })
                    .then(targetUser =>
                        Chat.create({ participants: [user.id, targetUser.id] })
                            .then(chat =>
                                Message.create({ chat: chat.id, author: user.id, message: 'hola!' })
                                    .then(message1 =>
                                        Message.create({ chat: chat.id, author: user.id, message: 'hola2' })
                                            .then(message2 =>
    ...
    it('fails on non-existing chat', () => {
        ...
        return User.create({ name: 'Samuele', surname: 'Spinetti', username: 'samu', email: 'samu@spine.com', password: '123123123' })
            .then(user =>
                User.create({ name: 'Liliana', surname: 'Deponte', username: 'lili', email: 'lili@deponte.com', password: '123123123' })
    ...
    it('fails on non existing author', () => {
        return User.create({ name: 'Samu', surname: 'Spinetti', username: 'samu', email: 'samu@spine.com', password: '123123123' })
            .then(user =>
                User.create({ name: 'Liliana', surname: 'Deponte', username: 'lili', email: 'lili@deponte.com', password: '123123123' })
                    .then(targetUser =>
                        Chat.create({ participants: [user.id, targetUser.id] })
